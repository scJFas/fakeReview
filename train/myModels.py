...
def model_CNN(filters = 32, act = "sigmoid"):
...
def model_CNN4(filters = 32):
    tf.keras.backend.set_floatx('float64')
    model = models.Sequential()
    model.add(tf.keras.Input(shape=(1,128,128), dtype="float64", name="inputLayer"))
    model.add(layers.Conv2D(filters, (3,64), (1,64), data_format="channels_first", activation="relu"))
    model.add(layers.MaxPool2D((2,2), data_format="channels_first"))
    model.add(layers.Flatten(data_format="channels_first"))
    model.add(layers.Dense(20, activation="relu"))
...
def model_CNN5():
    ...
    model = models.Sequential()
    model.add(tf.keras.Input(shape=(1, 128, 128), dtype="float64", name="inputLayer"))
    model.add(layers.Conv2D(32, (5, 5), (1, 1), data_format="channels_first", activation="relu"))
    model.add(layers.MaxPool2D((2, 2), data_format="channels_first"))
    model.add(layers.Flatten(data_format="channels_first"))
    model.add(layers.Dense(100, activation="relu"))
    model.add(layers.Dense(2))
